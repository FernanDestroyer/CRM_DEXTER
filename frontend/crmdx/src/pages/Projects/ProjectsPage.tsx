...
export default function ProjectsPage() {
  ...
  const handleDelete = async (id: string, name: string, e: React.MouseEvent) => {
    ...
    try { await apiClient.delete(`/proyectos/${id}`); setProjects((prev) => prev.filter((p) => p.id !== id)); }
  ...
  return <div className="space-y-6">
    ...
    <CreateProjectModal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} onSuccess={(newProject) => { setProjects((prev) => [newProject, ...prev]); setActiveProjectId(newProject.id); setIsCreateOpen(false); }} />
  </div>;
}
